...
def mapper(record):
		...
		is_flush = ( 
					...
					(counts['H'] == 5)) 
		is_4straight = False
		card4straight = 'A23456789TJQKA';
		for i in range(0, 11): 
			 if (counts[card4straight[i]] and 
						 counts[card4straight[i+1]] and 
						 counts[card4straight[i+2]] and 
						 counts[card4straight[i+3]]): 
					is_4straight = True 
					break
		...
		elif is_4straight and not is_straight and not is_flush: 
